...
def spending_by_workday(transactions_1: pd.DataFrame, date: str = None) -> dict:
    ...
    transactions_1['Дата операции'] = pd.to_datetime(transactions_1['Дата операции'], format='%d.%m.%Y %H:%M:%S')
    three_months_ago = datetime.strptime(date, '%d.%m.%Y %H:%M:%S') - timedelta(days=90)
    filtered_transactions = transactions_1[transactions_1['Дата операции'] >= three_months_ago]

    workdays = filtered_transactions[filtered_transactions['Дата операции'].dt.weekday < 5]
    weekends = filtered_transactions[filtered_transactions['Дата операции'].dt.weekday >= 5]
